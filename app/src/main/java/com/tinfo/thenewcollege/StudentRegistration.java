...
public class StudentRegistration extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        secorder=(Spinner)findViewById(R.id.spnrsection);
    ...
    private void registerUser() {
        ...
        String Section=secorder.getSelectedItem().toString().trim();
        String YearofStudying=Year.getSelectedItem().toString().trim();
        ...
        if (n.isEmpty()||n.length()>8) {
            studentname.setError("Name Required with maximum of 8 characters only");
        ...
        if (!Patterns.EMAIL_ADDRESS.matcher(em).matches()) {
            registermail.setError("Enter valid Email ID provided by College");
            registermail.requestFocus();
            return;
                ...
                .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                          User user = new User(n,em,d,Section,YearofStudying);
                            FirebaseDatabase.getInstance().getReference("Users").child(FirebaseAuth.getInstance().getUid())
                                    .setValue(user).addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {
                                    if (task.isSuccessful()) {
                                        Toast.makeText(StudentRegistration.this, "User has been Registered Successfully", Toast.LENGTH_LONG).show();
